            ...
            <ScrollReveal key={plot.size} delay={i * 0.15}>
              <div
                className={`relative h-full overflow-hidden rounded-3xl transition-all duration-500 ${
                  plot.popular
                    ? "ring-2 ring-forest-400 shadow-xl shadow-forest-400/10"
              ...
              >
                {plot.popular && (
                  <div className="absolute top-4 right-4 z-20 rounded-full bg-forest-400 px-3 py-1">
                    <span
                      className="text-xs font-bold text-forest-950"
                      style={{ fontFamily: "var(--font-heading)" }}
                    >
                      ПОПУЛЯРНЫЙ
                    </span>
                ...
                <div className="bg-forest-950/80 p-6 backdrop-blur-sm sm:p-8">
                  ...
                  <div className="mb-6">
                    ...
                    <p
                      className="text-3xl font-black text-forest-300 sm:text-4xl"
                      style={{ fontFamily: "var(--font-heading)" }}
                    >
                      от {plot.price} ₽
                    </p>
                  ...
                  <button
                    ...
                    className="w-full rounded-xl bg-forest-400 py-3.5 text-sm font-bold text-forest-950 transition-all hover:bg-forest-300 active:scale-[0.98]"
                  >
                    Узнать подробнее
                  </button>
                </div>
        ...
        <ScrollReveal delay={0.3}>
          <div className="mt-12 grid grid-cols-2 gap-3 sm:grid-cols-5 sm:gap-4">
            {benefits.map((b) => (
              <div
                key={b}
                className="flex items-center gap-2 rounded-xl bg-white/5 px-3 py-2.5 text-sm text-white/70 sm:px-4"
              >
